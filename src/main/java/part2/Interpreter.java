...
public class Interpreter {
    ...
    public Token getNextToken() throws Exception {
        ...
        if(currentChar.equals("*")){
            this.pos += 1;
            return new Token(ConstVal.MULTIPLY, currentChar);
        }
        if(currentChar.equals("/")){
            this.pos += 1;
            return new Token(ConstVal.DIVIDE, currentChar);
        }
        error();
        return new Token();
    }
    ...
    public int expr() throws Exception {
        ...
        if(op.type.equals(ConstVal.PLUS)){
            eat(ConstVal.PLUS);
        }
        else if(op.type.equals(ConstVal.MINUS)){
            eat(ConstVal.MINUS);
        }
        else if(op.type.equals(ConstVal.MULTIPLY)){
            eat(ConstVal.MULTIPLY);
        }
        else if(op.type.equals(ConstVal.DIVIDE)){
            eat(ConstVal.DIVIDE);
        }
